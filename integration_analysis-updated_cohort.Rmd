...
## Batch correction / integration using Seurat 3
...
# saveRDS(hl_relapse.anchors, file = "~/Documents/sc_HL_relapse/seurat/output/relapse-all_25_merged/updated_cohort/ hl_relaspe_25-anchors.rds") # 1GB

# read back in if necessary
hl_relapse.anchors <- readRDS("~/Documents/sc_HL_relapse/seurat/output/relapse-all_25_merged/split_by_chip/hl_relaspe_25-anchors.rds")
...
# saveRDS(hl_relapse.combined, file = "~/Documents/sc_HL_relapse/seurat/output/relapse-all_25_merged/updated_cohort/hl_relaspe_25-combined.rds") # 1GB
```

(optional:) analyse the individual datasets (relapse)
...
# Save object so that it can be loaded without having to re-run computationally intensive steps again
# saveRDS(hl_diag.anchors, file = "~/Documents/sc_HL_relapse/seurat/output/diagnostic_updated_16/hl_diag_22-anchors.rds") # 1GB

# read back in if necessary
hl_diag.anchors <- readRDS("~/Documents/sc_HL_relapse/seurat/output/diagnostic_updated_16/hl_diag_22-anchors.rds")
...
# saveRDS(hl_diag.combined, file = "~/Documents/sc_HL_relapse/seurat/output/diagnostic_updated_16/hl_diag_22-combined.rds") # 1GB
```

(optional:) analyse the individual datasets (diagnostic)

```{r}
hl_diag.combined <- readRDS("~/Documents/sc_HL_relapse/seurat/output/diagnostic_updated_16/hl_diag_22-combined.rds")
...
```{r}
hl_diag.combined <- readRDS("~/Documents/sc_HL_relapse/seurat/output/diagnostic_updated_16/hl_diag_22-combined.rds")
hl_relapse.combined <- readRDS("~/Documents/sc_HL_relapse/seurat/output/relapse-all_25_merged/updated_cohort/hl_relaspe_25-combined.rds")

# memory-intensive -- must perform on GSC project space:
...
hl.both.combined <- readRDS("~/Documents/sc_HL_relapse/seurat/output/updated-diag_plus_relapse/hl.both.combined.rds")
```
...
saveRDS(hl.both.combined, "~/Documents/sc_HL_relapse/seurat/output/updated-diag_plus_relapse/hl.integrated.clustered.rds")
...
write.table(hl.both.hvg, "~/Documents/sc_HL_relapse/seurat/output/updated-diag_plus_relapse/hl.both.integrated.hgv.txt", sep = "\t", quote = FALSE)
...
# test case to see how colours of heatmap changes
hl.integrated.scaled <- hl.both.combined@assays$integrated@data
...
for (c in 1:NUM_CLUSTERS) {
  ...
  barcodes[[c]] <- filter(clusters.df, clusters.df$clusters.char == c-1)
  expr[[c]] <- hl.integrated.markers.ordered[, colnames(hl.integrated.markers.ordered) %in% barcodes[[c]]$barcodes.cluster ]
  heatmaps[[c]] <- pheatmap(expr[[c]], 
