...
# 76,607 cells or ~3K cells / sample
...
saveRDS(hl_relapse.merged, file = "~/Documents/sc_HL_relapse/seurat/output/hl_relaspe_25-merged.rds")
```
...
ids <- c("04-32704", "09-25099", "10-39536", "12-39704", "14-22984", "15-31135", "17-29163", "17-37524", "07-19278", "10-16486", "11-11363", "13-35368", "14-34982", "17-11527", "17-30019", "08-23834", "10-20817", "12-10493", "14-20419", "15-29662", "17-12745", "17-33404")
...
# based on violoin plots, the vast majority of the dataset sits < 5% MT
hl_diag.merged <- subset (hl_diag.merged, subset = nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 5)

# 98,313 cells or 4,4K cells/sample
table(hl_relapse.combined$orig.ident) # get specific numbers based on sample id
...
# for both datasets, sample names are under the "Sample" column and the "batch" would be the "Chip" column
hl_diag.metadata <- read.table("~/Documents/sc_HL_relapse/data/primary/library_info-primary_cohort.txt", sep = "\t", header = TRUE, fill = TRUE)

# map batch information to orig.idents 
hl_diag.merged$batch <- plyr::mapvalues(
    x = hl_diag.merged$orig.ident, 
    from = hl_diag.metadata$Sample, 
    to = hl_diag.metadata$Chip
)

saveRDS(hl_diag.merged, file = "~/Documents/sc_HL_relapse/seurat/output/hl_diag.merged.rds")
```
...
```{r}
# Pre-batch-correction steps: filtering, normalization, finding variable genes
hl_relapse.merged <- NormalizeData(hl_relapse.merged)
...
hl_relapse.merged <- ScaleData(hl_relapse.merged)
hl_relapse.merged <- RunPCA(hl_relapse.merged, npcs = 30)

# UMAP and clustering
hl_relapse.merged <- RunUMAP(hl_relapse.merged, reduction = "pca", dims = 1:20)
hl_relapse.merged <- FindNeighbors(hl_relapse.merged, reduction = "pca", dims = 1:20)
hl_relapse.merged <- FindClusters(hl_relapse.merged, resolution = 0.5)

# Visualization
p.no_batch_correction.batch <- DimPlot(hl_relapse.merged, reduction = "umap", group.by = "batch")
p.no_batch_correction.celltype <- DimPlot(hl_relapse.merged, reduction = "umap", group.by = "celltype")
p.no_batch_correction.cluster <- DimPlot(hl_relapse.merged, reduction = "umap", label = TRUE)
```

## And same thing for diagostic dataset

```{r}
hl_diag.merged <- readRDS("~/Documents/sc_HL_relapse/seurat/output/hl_diag.merged.rds")

# Pre-batch-correction steps: filtering, normalization, finding variable genes
hl_diag.merged <- NormalizeData(hl_diag.merged)
hl_diag.merged <- FindVariableFeatures(hl_diag.merged, selection.method = "vst", nfeatures = 2000)

# Run standard workflow of visualization and clustering
hl_diag.merged <- ScaleData(hl_diag.merged)
hl_diag.merged <- RunPCA(hl_diag.merged, npcs = 30)

# UMAP and clustering
hl_diag.merged <- RunUMAP(hl_diag.merged, reduction = "pca", dims = 1:20)
hl_diag.merged <- FindNeighbors(hl_diag.merged, reduction = "pca", dims = 1:20)
hl_diag.merged <- FindClusters(hl_diag.merged, resolution = 0.5)

# Visualization
p.no_batch_correction.batch <- DimPlot(hl_diag.merged, reduction = "umap", group.by = "batch")
#p.no_batch_correction.celltype <- DimPlot(hl_diag.merged, reduction = "umap", group.by = "celltype")
p.no_batch_correction.cluster <- DimPlot(hl_diag.merged, reduction = "umap", label = TRUE)

```

## ###############################################
## Batch correction method 1: Seurat 3
## ###############################################
...
saveRDS(hl.both.combined, "~/Documents/sc_HL_relapse/seurat/output/diagnostic_plus_relapse/hl.both.combined.annotated.rds")
...
hl.both.combined <- readRDS("~/Documents/sc_HL_relapse/seurat/output/diagnostic_plus_relapse/hl.both.combined.annotated.rds")
...
de.methods <- c("wilcox", # NA's for p-values --> generic method
                ...
                "MAST", #  2 / 54 genes have p-value > 0 (and < 0.05) --> **the only scRNAseq-specific method
...
de.diag_vs_rel <- FindMarkers(hl.both.combined, ident.1 = "relapse", ident.2 = "diagnostic", 
                          test.use = "MAST",
                          min.pct = 0.25, logfc.threshold = 0.25)
#write.table(relapse.de, "~/Documents/sc_HL_relapse/seurat/output/diagnostic_plus_relapse/de_analysis/all.relapse.de.MAST.txt", sep = "\t", quote = FALSE)
...
## ###############################################
## Batch correction method 2: MNN Correct
## ###############################################

```{r}
library(batchelor)

# 1. Perform pre-batch-correction steps: filtering
#   and for each batch: normalization, (scaling?) and finding highly variable genes (HVG)
hl_relapse.merged[["percent.mt"]] <- PercentageFeatureSet(hl_relapse.merged, pattern = "^MT-")
hl_relapse.merged <- subset (hl_relapse.merged, subset = nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 20)

# split relapse cases by batch
hl_relapse.split <- SplitObject(hl_relapse.merged, split.by = "batch") # returns a list of Seurat objects
...
hl_relapse.sce <- vector('list', length(hl_relapse.split))
  
# Convert Seurat objects into SingleCellExperiment objects
for (i in 1:length(hl_relapse.split)) {
 hl_relapse.sce[[i]] <- as.SingleCellExperiment(hl_relapse.split[[i]]) 
}

# 2. Then, using MNN Correct, using the HVG as input into MNN, batch-correct the data
hl_relapse.mnn_correct <- mnnCorrect(hl_relapse.sce[[1]],
                                     hl_relapse.sce[[2]],
                                     hl_relapse.sce[[3]],
                                     hl_relapse.sce[[4]],
                                     hl_relapse.sce[[5]])

# 3. Perform PCA and visualization
...
# Visualization
p.MNN.batch <- DimPlot(hl_relapse.merged, reduction = "umap", group.by = "batch")
p.MNN.cluster <- DimPlot(hl_relapse.merged, reduction = "umap", label = TRUE)


```

## ###############################################
## Batch correction method 3: FastMNN
## ###############################################

```{r}
hl_relapse.merged <- readRDS("~/Documents/sc_HL_relapse/seurat/output/hl_relaspe_25-merged.rds")
hl_relapse.merged <- NormalizeData(hl_relapse.merged)
hl_relapse.merged <- FindVariableFeatures(hl_relapse.merged)
hl_relapse.merged <- RunFastMNN(object.list = SplitObject(hl_relapse.merged, split.by = "batch"))
hl_relapse.merged <- RunUMAP(hl_relapse.merged, reduction = "mnn", dims = 1:30)
hl_relapse.merged <- FindNeighbors(hl_relapse.merged, reduction = "mnn", dims = 1:30)
hl_relapse.merged <- FindClusters(hl_relapse.merged)
saveRDS(hl_relapse.merged, file = "~/Documents/sc_HL_relapse/seurat/output/hl_relaspe_25-merged-fastMNN.rds")
#hl_relapse.merged <- readRDS("~/Documents/sc_HL_relapse/seurat/output/hl_relaspe_25-merged-fastMNN.rds")

#DimPlot(hl_relapse.merged, group.by = c("batch", "ident", "celltype"), ncol = 3)
p.fastMNN.batch <- DimPlot(hl_relapse.merged, reduction = "umap", group.by = "batch")
p.fastMNN.cluster <- DimPlot(hl_relapse.merged, reduction = "umap", label = TRUE)
p.fastMNN.celltype <- DimPlot(hl_relapse.merged, reduction = "umap", group.by = "celltype")
```

## And same thing for diagnostic

```{r}
hl_diag.merged <- readRDS("~/Documents/sc_HL_relapse/seurat/output/hl_diag.merged.rds")

hl_diag.merged <- NormalizeData(hl_diag.merged)
hl_diag.merged <- FindVariableFeatures(hl_diag.merged)
hl_diag.merged <- RunFastMNN(object.list = SplitObject(hl_diag.merged, split.by = "batch"))
hl_diag.merged <- RunUMAP(hl_diag.merged, reduction = "mnn", dims = 1:30)
hl_diag.merged <- FindNeighbors(hl_diag.merged, reduction = "mnn", dims = 1:30)
hl_diag.merged <- FindClusters(hl_diag.merged)
#saveRDS(hl_diag.merged, file = "~/Documents/sc_HL_relapse/seurat/output/hl_relaspe_25-merged-fastMNN.rds")
#hl_diag.merged <- readRDS("~/Documents/sc_HL_relapse/seurat/output/hl_relaspe_25-merged-fastMNN.rds")

#DimPlot(hl_diag.merged, group.by = c("batch", "ident", "celltype"), ncol = 3)
p.fastMNN.batch <- DimPlot(hl_diag.merged, reduction = "umap", group.by = "batch")
p.fastMNN.cluster <- DimPlot(hl_diag.merged, reduction = "umap", label = TRUE)
#p.fastMNN.celltype <- DimPlot(hl_relapse.merged, reduction = "umap", group.by = "celltype")

```

## ###############################################
## Batch correction method 4: Harmony
## ###############################################

```{r}
hl_relapse.merged <- readRDS("~/Documents/sc_HL_relapse/seurat/output/hl_relaspe_25-merged.rds")
hl_relapse.merged <- NormalizeData(hl_relapse.merged) %>% FindVariableFeatures() %>% ScaleData() %>% RunPCA()
hl_relapse.merged <- RunHarmony(hl_relapse.merged, group.by.vars = "batch")
hl_relapse.merged <- RunUMAP(hl_relapse.merged, reduction = "harmony", dims = 1:30)
hl_relapse.merged <- FindNeighbors(hl_relapse.merged, reduction = "harmony", dims = 1:30) %>% FindClusters()
saveRDS(hl_relapse.merged, file = "~/Documents/sc_HL_relapse/seurat/output/hl_relaspe_25-merged-harmony.rds")
#hl_relapse.merged <- readRDS("~/Documents/sc_HL_relapse/seurat/output/hl_relaspe_25-merged-harmony.rds")

#DimPlot(hl_relapse.merged, group.by = c("batch", "ident", "celltype"), ncol = 3)
p.harmony.batch <- DimPlot(hl_relapse.merged, reduction = "umap", group.by = "batch")
p.harmony.cluster <- DimPlot(hl_relapse.merged, reduction = "umap", label = TRUE)
p.harmony.celltype <- DimPlot(hl_relapse.merged, reduction = "umap", group.by = "celltype")
...
hl_diag.merged <- NormalizeData(hl_diag.merged) %>% FindVariableFeatures() %>% ScaleData() %>% RunPCA()
hl_diag.merged <- RunHarmony(hl_diag.merged, group.by.vars = "batch")
hl_diag.merged <- RunUMAP(hl_diag.merged, reduction = "harmony", dims = 1:30)
hl_diag.merged <- FindNeighbors(hl_diag.merged, reduction = "harmony", dims = 1:30) %>% FindClusters()
#saveRDS(hl_diag.merged, file = "~/Documents/sc_HL_relapse/seurat/output/hl_diag_25-merged-harmony.rds")
#hl_diag.merged <- readRDS("~/Documents/sc_HL_relapse/seurat/output/hl_relaspe_25-merged-harmony.rds")

#DimPlot(hl_diag.merged, group.by = c("batch", "ident", "celltype"), ncol = 3)
p.harmony.batch <- DimPlot(hl_diag.merged, reduction = "umap", group.by = "batch")
p.harmony.cluster <- DimPlot(hl_diag.merged, reduction = "umap", label = TRUE)
#p.harmony.celltype <- DimPlot(hl_diag.merged, reduction = "umap", group.by = "celltype")
```

## ###############################################
## Batch correction method 5: LIGER
## ###############################################

```{r}
# will have to try another day - having trouble installing the package
...
### And same thing for diagnostic dataset

hl_diag.merged[["percent.mt"]] <- PercentageFeatureSet(hl_diag.merged, pattern = "^MT-")
head(hl_diag.merged@meta.data, 5)
...
# read back in if necessary
hl_relapse.merged.sct <- readRDS("~/Documents/sc_HL_relapse/seurat/output/hl_relaspe_25-merged-sctransform_workflow.rds")
```
...
hl_relapse.merged.sct <- RenameIdents(hl_relapse.merged.sct, 
                           `0` = "B cells", 
                           `1` = "B cells", 
                           `2` = "Helper T cells?", 
                           `3` = "B cells",
