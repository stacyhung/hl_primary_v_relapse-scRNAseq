...
```{r}
NUM_CLUSTERS = length(unique(hl.both.combined$celltype))
...
# initialize empty vector of results
de.diag_vs_rel.clusters <- vector('list', NUM_CLUSTERS)

# find DE genes between diagnostic vs. all relapse cases WITHIN EACH CLUSTER / CELL TYPE
for (i in 1:NUM_CLUSTERS) {
   tempResults <- FindMarkers(hl.both.combined, assay = "RNA",
                          ident.1 = comparators[i], ident.2 = comparators[i+1], 
   ...
   tempResults$comparison <- paste(comparators[i], "_VS_", comparators[i+1], sep = "")
   ...
   de.diag_vs_rel.clusters[[i]] <- tempResults
   i = i + 2
}

# Repeat cluster-specific comparisons for:
