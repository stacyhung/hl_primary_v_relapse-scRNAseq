...
get_de_genes <- function(comparators) {
  ...
  for (i in 1:NUM_CLUSTERS) {
    index.grp1 <- (i*2)-1
    index.grp2 <- i*2
    tempResults <- FindMarkers(hl.both.combined, assay = "RNA",
                            ident.1 = comparators[index.grp1], ident.2 = comparators[index.grp2], 
                            test.use = "MAST", min.pct = 0.25, logfc.threshold = 0.25)
    # make gene a column
...
de.diag_vs_rel.clusters <- get_de_genes(comparators = comparators)
de.diag_vs_rel.clusters.df <- merge_de_results(de.diag_vs_rel.clusters)
write.table(de.diag_vs_rel.clusters.df, "~/Documents/sc_HL_relapse/seurat/output/diagnostic_plus_relapse/de_analysis/de.diag_vs_rel.clusters.txt", sep = "\t", quote = FALSE, row.names = FALSE)
```
...
de.diag_vs_refractory.clusters <- get_de_genes(comparators = comparators)
de.diag_vs_refractory.clusters.df <- merge_de_results(de.diag_vs_refractory.clusters)
write.table(de.diag_vs_refractory.clusters.df, "~/Documents/sc_HL_relapse/seurat/output/diagnostic_plus_relapse/de_analysis/de.diag_vs_refractory.clusters.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
# perform the DE analysis
de.diag_vs_early_relapse.clusters <- get_de_genes(comparators = comparators)
de.diag_vs_early_relapse.clusters.df <- merge_de_results(de.diag_vs_early_relapse.clusters)
write.table(de.diag_vs_early_relapse.clusters.df, "~/Documents/sc_HL_relapse/seurat/output/diagnostic_plus_relapse/de_analysis/de.diag_vs_early_relapse.clusters.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
# perform the DE analysis
de.diag_vs_late_relapse.clusters <- get_de_genes(comparators = comparators)
de.diag_vs_late_relapse.clusters.df <- merge_de_results(de.diag_vs_late_relapse.clusters)
write.table(de.diag_vs_late_relapse.clusters.df, "~/Documents/sc_HL_relapse/seurat/output/diagnostic_plus_relapse/de_analysis/de.diag_vs_late_relapse.clusters.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
comparators <- comparators[ !grepl("Plasma cells", comparators)]
# perform the DE analysis
de.refractory_vs_relapse.clusters <- get_de_genes(comparators = comparators)
de.refractory_vs_relapse.clusters.df <- merge_de_results(de.refractory_vs_relapse.clusters)
write.table(de.refractory_vs_relapse.clusters.df, "~/Documents/sc_HL_relapse/seurat/output/diagnostic_plus_relapse/de_analysis/de.refractory_vs_relapse.clusters.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
comparators <- comparators[ !grepl("_diagnostic", comparators)]
# perform the DE analysis
de.early_vs_late_relapse.clusters <- get_de_genes(comparators = comparators)
de.early_vs_late_relapse.clusters.df <- merge_de_results(de.early_vs_late_relapse.clusters)
write.table(de.early_vs_late_relapse.clusters.df, "~/Documents/sc_HL_relapse/seurat/output/diagnostic_plus_relapse/de_analysis/de.early_vs_late_relapse.clusters.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
## Visualize receptor-ligand interactions
...
# read the data
expr.df <- t(read.table("~/Documents/sc_HL_relapse/seurat/output/diagnostic_plus_relapse/hl_combined.norm_expr.txt", sep = ",", header = TRUE, stringsAsFactors = FALSE))

# # make genes column names and row names cell barcodes
# expr.df <- t(expr.df)

# add column (e.g. "cell_type" to denote cluster annotations for each cell)
case.type <- hl.both.combined$case.type

## highly expressed ligand-receptor pairs

# find top 50 percent highly expressed genes
highly_exprs_genes <- rawParse(expr.df, top_genes = 50, stats='mean')
