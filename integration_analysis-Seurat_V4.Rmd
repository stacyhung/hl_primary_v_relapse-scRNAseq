...
hl_rln.orig.idents <- as.data.frame(hl_rln.merged$orig.ident)
hl_rln.names <- as.character(hl_rln.orig.idents$`hl_rln.merged$orig.ident`)

hl_rln.merged$batch <- plyr::mapvalues(
    x = hl_rln.names, 
    from = as.character(final.cohort$Sample), 
    to = as.character(final.cohort$Batch)
)
hl_rln.merged$chemistry <- plyr::mapvalues(
    x = hl_rln.names, 
    from = as.character(final.cohort$Sample), 
...
# map batch information to orig.idents 
hl_relapse.orig.idents <- as.data.frame(hl_relapse.merged$orig.ident)
hl_relapse.names <- as.character(hl_relapse.orig.idents$`hl_relapse.merged$orig.ident`)
...
hl_relapse.merged$chemistry <- plyr::mapvalues(
    x = hl_relapse.names, 
    from = as.character(final.cohort$Sample), 
    to = as.character(final.cohort$X3_prime_assay)
)
...
# map batch information to orig.idents 
hl_diag.orig.idents <- as.data.frame(hl_diag.merged$orig.ident)
hl_diag.names <- as.character(hl_diag.orig.idents$`hl_diag.merged$orig.ident`)
...
hl_diag.merged$chemistry <- plyr::mapvalues(
    x = hl_diag.names, 
    from = as.character(final.cohort$Sample), 
    to = as.character(final.cohort$X3_prime_assay)
)
...
hl_rln.merged <- readRDS("~/Documents/sc_HL_relapse/seurat/output/LATEST/RLN-merged.rds")
hl_diag.merged <- readRDS("~/Documents/sc_HL_relapse/seurat/output/LATEST/hl_diag.merged.rds")
hl_relapse.merged <- readRDS("~/Documents/sc_HL_relapse/seurat/output/LATEST/hl_relapse-merged.rds")
...
# previously, batch 10 and 12 were problematic: produced error "NaNs producedError in while ((it <- it + 1) < limit && abs(del) > eps) { : missing value where TRUE/FALSE needed"
# hl_all.split[[12]] <- SCTransform(hl_all.split[[12]], vars.to.regress = "percent.mt", verbose = FALSE) 
...
saveRDS(hl_all.split, "~/Documents/sc_HL_relapse/seurat/output/LATEST/hl_all.split.sctransform.rds")
...
hl_all.features <- SelectIntegrationFeatures(object.list = hl_all.split, nfeatures = 3000)

# implement `future` framework for parallelization
plan("multiprocess") # allow parallelization on maximum cores
options(future.globals.maxSize = 15728640000) # allow 15G mem (15*1000*1024^2)
hl_all.split <- PrepSCTIntegration(object.list = hl_all.split, anchor.features = hl_all.features)

# Specify reference - one batch per cohort (choose batch with highest cell count)
#   which(names(hl_all.split) == "CHIP008" ==> 7
#   which(names(hl_all.split) == "CHIP020" ==> 10
#   which(names(hl_all.split) == "CHIP013" ==> 1
reference.dataset <- c(7, 10, 1)

# Previously, used 2000 features (default) with log normalization (default, not SCT) and dims=1:20 (default is 1:30).
hl_all.anchors <- FindIntegrationAnchors(object.list = hl_all.split, 
                                         anchor.features = hl_all.features, 
                                         reference = reference.dataset,
                                         normalization.method = "SCT")

# saveRDS(hl_all.anchors, "~/Documents/sc_HL_relapse/seurat/output/LATEST/hl_all.anchors.rds")

# !!!!! NEW !!!! You can specify the order of integration with the sample.tree parameter
hl_all.combined <- IntegrateData(anchorset = hl_all.anchors, normalization.method = "SCT")
# check order of integration - these don't work
Tool(object = hl_all.combined, slot = "Integration")@sample.tree
hl_all.combined@tools$Integration@sample.tree
```
...
hl_all.combined <- readRDS("~/Documents/sc_HL_relapse/seurat/output/V4_LATEST/hl_all.combined.sctransform.rds")
...
plots <- DimPlot(hl_all.combined, group.by = c("batch", "chemistry"))
...
saveRDS(hl_all.combined, "~/Documents/sc_HL_relapse/seurat/output/V4_LATEST/hl_all.integrated.clustered.rds")
...
markers.to.plot <- c("CD19", "CD27", "IGHD", "IGHG1", "IGHG2", "IGHG3", "IGHG4", "IGHGP", "IGHM", "MS4A1", "CD38", # B cell
                     "CD28", "CD40LG", "ICOS", "TNFRSF18", "TNFRSF8", # Co-stimulatory molecule
                     "IFNG", "IL2", "IL4", # Cytokine / chemokine
                     "GZMA", "GZMB", "GZMK", # Effector molecule
                     "CTLA4", "HAVCR2", "LAG3", "PDCD1", "TIGIT", # Inhibitory receptor
                     ...
                     "MKI67", # Proliferation
                     "CD3D", "CD4", "CD8B", # T cell
                     "BCL6", "CCR4", "CXCL13", "GATA3", "KLRB1", "TBX21", "CXCR5", # T helper
                     "EOMES", "ID2", # TF
                     "FOXP3", "IKZF2", "IL2RA" # T reg
                     )
...
# remove genes that are not part of HVG list (IL4, IDO1, SDC1, NRP1)
...
p4 <- pheatmap(hl.integrated.components.cluster.avgs.ordered, 
         ...
         angle_col = 45, 
         gaps_row = c(10,15,17,20,25,27,30,31,32,33,36,42,44), border_color = NA)

p4.rotated <- pheatmap(t(hl.integrated.components.cluster.avgs.ordered), 
         annotation_col = component.mappings,
         cluster_cols = FALSE, 
         annotation_names_col = FALSE, 
         gaps_col = c(10,15,17,20,25,27,30,31,32,33,36,42,44), 
         angle_col = 45, 
         border_color = NA)
...
for (c in 1:NUM_CLUSTERS) {
  ...
  filename <- paste("~/Documents/sc_HL_relapse/seurat/figures/V4_LATEST/heatmaps/sctransform/heatmap-sctransform-cluster", 
                    c-1, ".pdf", sep = "")
  ...
  heatmaps[[c]] <- pheatmap(expr.markers[[c]], 
         ...
         annotation_names_row = FALSE, 
         angle_col = 45, 
         gaps_row = c(11,16,18,21,26,28,31,32,33,34,37,44,46,49), 
...
hl_all.combined <- RenameIdents(hl_all.combined,
                                ...
                                `14` = "Naive B cell",
                                `15` = "T-reg",
                                `16` = "GCB and T cell proliferation",
                                `17` = "Naive CD8 cell",
                                `18` = "NK cell",
                                `19` = "Naive B cell",
                                `20` = "Plasma cell",
...
saveRDS(hl_all.combined, "~/Documents/sc_HL_relapse/seurat/output/V4_LATEST/hl_all.integrated.annotated.rds")
...
p <- ggplot(sample.celltype_num, 
  ...
  scale_fill_manual(name = "Cell Identity",
                    values = c("Naive B cell" = "darkslateblue",
                               ...
                               "T helper" = "aquamarine3",
                               "T-reg" = "hotpink3", 
                               "CTL" = "olivedrab4",
                               "B cell"="bisque3",
                               "Naive CD8 cell"="royalblue1",
                               "GCB and T cell proliferation"="firebrick3",
                               "NK cell"="darkorange",
                               "Plasma cell"="mediumpurple1",
                               "pDC"="green3",
                               "macrophage"="magenta3"
                               )) +
...
# map batch information to orig.idents
hl_all.orig.idents <- as.data.frame(hl_all.combined$orig.ident)
hl_all.names <- as.character(hl_all.orig.idents$`hl_all.combined$orig.ident`)

# pathology
hl_all.combined$biology <- plyr::mapvalues(x = hl_all.names, 
    from = as.character(final.cohort$Sample), to = as.character(final.cohort$Biology))

# pathology
hl_all.combined$pathology <- plyr::mapvalues(x = hl_all.names, 
    from = as.character(final.cohort$Sample), to = as.character(final.cohort$Pathology))

# EBV status
hl_all.combined$ebv <- plyr::mapvalues(x = hl_all.names, 
    from = as.character(final.cohort$Sample), to = as.character(final.cohort$EBVStatus))

# gender
hl_all.combined$sex <- plyr::mapvalues(x = hl_all.names, 
    from = as.character(final.cohort$Sample), to = as.character(final.cohort$Sex))

# age
hl_all.combined$age <- plyr::mapvalues(x = hl_all.names, 
    from = as.character(final.cohort$Sample), to = as.character(final.cohort$Age))

# stage
hl_all.combined$stage <- plyr::mapvalues(x = hl_all.names, 
    from = as.character(final.cohort$Sample), to = as.character(final.cohort$Stage))

# early relapse status
hl_all.combined$earlyRelapse <- plyr::mapvalues(x = hl_all.names, 
    from = as.character(final.cohort$Sample), to = as.character(final.cohort$EarlyRelapse))

# refractory status
hl_all.combined$refractory <- plyr::mapvalues(x = hl_all.names, 
    from = as.character(final.cohort$Sample), to = as.character(final.cohort$Refractory))

saveRDS(hl_all.combined, "~/Documents/sc_HL_relapse/seurat/output/V4_LATEST/hl_all.integrated.annotated.rds")

# read back in as needed
hl_all.combined <- readRDS("~/Documents/sc_HL_relapse/seurat/output/V4_LATEST/hl_all.integrated.annotated.rds")
```
...
# though unecessary for this scenario, store number of cells for each comparator
...
# remove results that have an adjusted p-value of 1
de.results <- filter(de.results, de.results$p_val_adj < 1)
...
VP_by_gene <- function(gene_list, file_name, comp_grps, cluster_or_cell_types, width, height, isCluster, earlyRelapse, biology){
...
## TEST ##
# VP_by_gene(gene_list = c("GZMA", "GZMB", "GZMK", "PRF1", "CXCR3", "IFNG", "HAVCR2", "LAG3", "PDCD1", "CXCR4"),
#          file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/CTL-diag_vs_rel_TEST", 
#          comp_grps = list(c("diagnostic", "relapse")),
#          cluster_or_cell_types = c("CTL"), 
#          width = 20, height = 10,
#          isCluster = FALSE)
...
VP_by_gene(gene_list = c("GZMB", "CD163", "IL3RA", "HLA-DRB5"),
         ...
         width=10, height=10, 
         isCluster = FALSE, earlyRelapse = FALSE)
...
VP_by_gene(gene_list = c("GZMA", "GZMB", "GZMK", "PRF1", "CXCR3", "IFNG"),
         ...
         comp_grps = list(c("diagnostic", "relapse")), 
         cluster_or_cell_types = c("NK cells"), 
         width=20, height=10, 
         isCluster = FALSE, earlyRelapse = FALSE)

# 3. Treg cluster (cluster 4): LAG3, FOXP3, TIM3 (-> HAVCR2), CXCR3, PD-1 (-> PDCD1), Ki67 (-> MKI67), CD30 (-> TNFRSF8), TOX, CXCR4
VP_by_gene(gene_list = c("LAG3", "FOXP3", "HAVCR2", "CXCR3", "PDCD1", "MKI67", "TNFRSF8", "TOX", "CXCR4", "CD80", "CD86"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/Treg_cluster4-diag_vs_rel", 
         comp_grps = list(c("diagnostic", "relapse")), 
         cluster_or_cell_types = c("4"), 
         width=20, height=10, 
         isCluster = TRUE, earlyRelapse = FALSE)

# 5. Helper T cell (cluster 1): LAG3, CXCR5, TIM3 (HAVCR2), CXCR3, PD-1 (PDCD1), Ki67 (MKI67), CD30 (TNFRSF8), CXCL13, TOX, CD28, IL-4 (IL4), CXCR4
VP_by_gene(gene_list = c("LAG3", "CXCR5", "HAVCR2", "CXCR3", "PDCD1", "MKI67", "TNFRSF8", "CXCL13", "TOX", "CD28", "IL4", "CXCR4", "CD80", "CD86"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/HelperT_cluster1-diag_vs_rel", 
         comp_grps = list(c("diagnostic", "relapse")), 
         cluster_or_cell_types = c("1"), 
         width=20, height=10, 
         isCluster = TRUE, earlyRelapse = FALSE)

# 7. CTL (cluster 8): GZMA, GZMB, GZMK, PRF1, CXCR3, IFNG, TIM3 (HAVCR2), LAG3, PD1 (PDCD1), CXCR4
VP_by_gene(gene_list = c("GZMA", "GZMB", "GZMK", "PRF1", "CXCR3", "IFNG", "HAVCR2", "LAG3", "PDCD1", "CXCR4", "CD80", "CD86"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/CTL_cluster8-diag_vs_rel", 
         comp_grps = list(c("diagnostic", "relapse")), 
...
# 1. pDC cluster: GZB, CD163, CD123 (-> IL3RA), HLA-DRB5
VP_by_gene(gene_list = c("GZMB", "CD163", "IL3RA", "HLA-DRB5"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/pDC-diag_vs_early_rel", 
         comp_grps = list(c("diagnostic", "earlyRelapse")), 
         cluster_or_cell_types = c("Plasmacytoid DCs"), 
         width=10, height=10, 
         isCluster = FALSE, earlyRelapse = TRUE)

# 2. Macrophage cluster: CD163, PD-L1 (-> CD274), CD123 (-> IL3RA), CD80, CD86, CXCL9, CXCL10, CXCL11, CD40, CCL22, TIM3 (-> HAVCR2), HLA-DRB5
VP_by_gene(gene_list = c("CD163", "CD274", "IL3RA", "CD80", "CD86", "CXCL9", "CXCL10", "CXCL11", "CD40", "CCL22", "HAVCR2", "HLA-DRB5"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/macrophage-diag_vs_early_rel", 
         comp_grps = list(c("diagnostic", "earlyRelapse")), 
         cluster_or_cell_types = c("Macrophage"), 
         width=20, height=10, 
         isCluster = FALSE, earlyRelapse = TRUE)
...
# 6. Helper T cell (cell type): LAG3, CXCR5, TIM3 (HAVCR2), CXCR3, PD-1 (PDCD1), Ki67 (MKI67), CD30 (TNFRSF8), CXCL13, TOX, CD28, IL-4 (IL4), CXCR4
VP_by_gene(gene_list = c("LAG3", "CXCR5", "HAVCR2", "CXCR3", "PDCD1", "MKI67", "TNFRSF8", "CXCL13", "TOX", "CD28", "IL4", "CXCR4", "CD80", "CD86"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/HelperT-diag_vs_early_rel", 
         comp_grps = list(c("diagnostic", "earlyRelapse")), 
         cluster_or_cell_types = c("Helper T cells"), 
         width=20, height=10, 
         isCluster = FALSE, earlyRelapse = TRUE)
...
# 8. CTL (cell type): GZMA, GZMB, GZMK, PRF1, CXCR3, IFNG, TIM3 (HAVCR2), LAG3, PD1 (PDCD1), CXCR4
VP_by_gene(gene_list = c("GZMA", "GZMB", "GZMK", "PRF1", "CXCR3", "IFNG", "HAVCR2", "LAG3", "PDCD1", "CXCR4", "CD80", "CD86"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/CTL-diag_vs_early_rel", 
         comp_grps = list(c("diagnostic", "earlyRelapse")), 
         cluster_or_cell_types = c("CTL"), 
         width=20, height=10, 
         isCluster = FALSE, earlyRelapse = TRUE)
...
# 9. NK cells: GZMA, GZMB, GZMK, PRF1, CXCR3, IFNG
VP_by_gene(gene_list = c("GZMA", "GZMB", "GZMK", "PRF1", "CXCR3", "IFNG"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/NK-diag_vs_early_rel", 
         comp_grps = list(c("diagnostic", "earlyRelapse")), 
         cluster_or_cell_types = c("NK cells"), 
         width=20, height=10, 
         isCluster = FALSE, earlyRelapse = TRUE)
...
# 3. Treg cluster (cluster 4): LAG3, FOXP3, TIM3 (-> HAVCR2), CXCR3, PD-1 (-> PDCD1), Ki67 (-> MKI67), CD30 (-> TNFRSF8), TOX, CXCR4
VP_by_gene(gene_list = c("LAG3", "FOXP3", "HAVCR2", "CXCR3", "PDCD1", "MKI67", "TNFRSF8", "TOX", "CXCR4", "CD80", "CD86"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/Treg_cluster4-diag_vs_early_rel", 
         comp_grps = list(c("diagnostic", "earlyRelapse")), 
         cluster_or_cell_types = c("4"), 
         width=20, height=10, 
...
# 5. Helper T cell (cluster 1): LAG3, CXCR5, TIM3 (HAVCR2), CXCR3, PD-1 (PDCD1), Ki67 (MKI67), CD30 (TNFRSF8), CXCL13, TOX, CD28, IL-4 (IL4), CXCR4
VP_by_gene(gene_list = c("LAG3", "CXCR5", "HAVCR2", "CXCR3", "PDCD1", "MKI67", "TNFRSF8", "CXCL13", "TOX", "CD28", "IL4", "CXCR4", "CD80", "CD86"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/HelperT_cluster1-diag_vs_early_rel", 
         comp_grps = list(c("diagnostic", "earlyRelapse")), 
         cluster_or_cell_types = c("1"), 
         width=20, height=10, 
         isCluster = TRUE, earlyRelapse = TRUE)
...
# 7. CTL (cluster 8): GZMA, GZMB, GZMK, PRF1, CXCR3, IFNG, TIM3 (HAVCR2), LAG3, PD1 (PDCD1), CXCR4
VP_by_gene(gene_list = c("GZMA", "GZMB", "GZMK", "PRF1", "CXCR3", "IFNG", "HAVCR2", "LAG3", "PDCD1", "CXCR4", "CD80", "CD86"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/CTL_cluster8-diag_vs_early_rel", 
         comp_grps = list(c("diagnostic", "earlyRelapse")), 
         cluster_or_cell_types = c("8"), 
         width=20, height=10, 
         isCluster = TRUE, earlyRelapse = TRUE)
...
# 1. pDC cluster: GZB, CD163, CD123 (-> IL3RA), HLA-DRB5
VP_by_gene(gene_list = c("GZMB", "CD163", "IL3RA", "HLA-DRB5"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/pDC-HL_vs_RLN", 
         comp_grps = list(c("HL", "RLN")), 
         cluster_or_cell_types = c("Plasmacytoid DCs"), 
         width=10, height=10, 
         isCluster = FALSE, earlyRelapse = FALSE, biology = TRUE)
...
# define working directory:
work.dir <- "~/Documents/sc_HL_relapse/seurat/output/LATEST/DE_analysis/min_3_cells_per_group/"
