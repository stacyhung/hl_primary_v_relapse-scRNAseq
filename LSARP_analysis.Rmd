...
hl.v3 <- subset(hl.v3, subset = nFeature_RNA > 200 & nFeature_RNA < 3000 & percent.mt < 25) # 55,078 cells / 17 samples
...
plan(multicore) # allow parallelization on maximum cores (32 on gphost)
...
#   which(names(hl.split) == "CHIP020") ==> 10
...
See http://127.0.0.1:16348/library/celldex/doc/userguide.html#2_General-purpose_references for all references
...
singler_data <- as.SingleCellExperiment(hl_all.combined)
# Get cell type information
cell <- SingleR(singler_data, ref_data, labels = ref_data$label.main) 
cell2 <- SingleR(singler_data, ref_data, labels = ref_data$label.fine)
cell3 <- SingleR(singler_data, ref_data, labels = ref_data$label.ont)
# Store cell type information into Seurat Object
hl_all.combined[['SingleR.HPCA.labels.main']] <- cell$labels # what about cell$pruned.labels or cell$first.labels?
hl_all.combined[['SingleR.HPCA.labels.pruned']] <- cell$pruned.labels
...
p <- VlnPlot(hl_all.combined, features = v2_v_v3.de.sig_genes,
