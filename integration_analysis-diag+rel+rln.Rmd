...
get_de_genes <- function(comparators, de.test, cell_counts.df) {
  ...
  low_cell_counts.comparators <- cell_counts.df[ cell_counts.df$Freq < 100, ]$Var1
  ...
  for (i in 1:NUM_CLUSTERS) {
    ...
    if ( !(comparators[index.grp1] %in% low_cell_counts.comparators) && 
        ...
        tempResults <- FindMarkers(hl_all.combined, 
                                   ...
                                   min.pct = 0.25, 
                                   min.cells.group = 100, # minimum cells in (both? or at least one) groups
                                   logfc.threshold = 0)
        # make gene a column
...
# if reading back from GSC projects space:
de.diag_vs_rel.clusters.df <- read.table("~/Documents/sc_HL_relapse/seurat/output/LATEST/DE_analysis/")
```
...
comparators <- comparators[ !grepl("_(relapseNotRefractory|NA)", comparators)]
# store number of cells for each comparator
...
comparators <- comparators[ !grepl("_(lateRelapse|NA)", comparators)]
...
comparators <- comparators[ !grepl("_(earlyRelapse|NA)", comparators)]
...
comparators <- comparators[ !grepl("_(diagnostic|NA)", comparators)]
# store number of cells for each comparator
cell_counts.df <- as.data.frame(table(hl_all.combined$celltype.earlyrelapse))
