...
VP_by_gene <- function(gene_list, file_name, comp_grps, cluster_or_cell_types, width, height, isCluster, earlyRelapse){
...
VP_by_gene(gene_list = c("LAG3", "FOXP3", "HAVCR2", "CXCR3", "PDCD1", "MKI67", "TNFRSF8", "TOX", "CXCR4"),
...
VP_by_gene(gene_list = c("LAG3", "CXCR5", "HAVCR2", "CXCR3", "PDCD1", "MKI67", "TNFRSF8", "CXCL13", "TOX", "CD28", "IL4", "CXCR4"),
...
## First version of violin plots (Seurat style) using p-values
...
# Function to create a grid of violin plots based on:
#     gene_signature:   vector of genes to plot data for
#     file_name:        absolute file path and file prefix for plot to be printed to
#     test_sign:        vector of the two groups (or vector of pairs) to show signficance p-value for
#     y_max:            highest y value to show
#     curr_idents:      which cluster / cell type to retrieve expression from
#     width:            width to print plot
#     height:           height to print plot
vp_case1 <- function(gene_signature, file_name, test_sign, y_max, cluster_or_cell_types, width, height){
  plot_one_gene <- function(gene){
    VlnPlot(hl_all.combined, features = gene,
            pt.size = 0,
            group.by = "case.type", idents = cluster_or_cell_types,
      ...
      geom_bloxplot(fill="white")
  }
  purrr::map(gene_signature, plot_one_gene) %>% cowplot::plot_grid(plotlist = .)
  file_name <- paste(file_name, ".png", sep = "")
  ggsave(file_name, width = width, height = height)
}
...
# TEST: CTL (cell type): GZMA, GZMB, GZMK, PRF1, CXCR3, IFNG, TIM3 (HAVCR2), LAG3, PD1 (PDCD1), CXCR4
VP_by_gene(gene_signature = c("GZMA", "GZMB", "GZMK", "PRF1", "CXCR3", "IFNG", "HAVCR2", "LAG3", "PDCD1", "CXCR4"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/CTL-diag_vs_rel_TEST", 
...
vp_case1(gene_signature = c("GZMB", "CD163", "IL3RA", "HLA-DRB5"),
         file_name = "~/Documents/sc_HL_relapse/seurat/figures/LATEST/violin_with_p_vals/pDC-diag_vs_rel_v2", 
         test_sign = list(c("diagnostic", "relapse")), 
         y_max = 7.5, 
         c("Plasmacytoid DCs"), 10, 10)

# test_sign = list(c("diagnostic", "relapse"), c("diagnostic", "RLN"), c("relapse", "RLN")), 
...
vp_case1 <- function(gene_signature, file_name, test_sign, y_max, curr_idents, width, height){
  plot_case1 <- function(signature){
    VlnPlot(hl_all.combined, features = signature,
            pt.size = 0,
            group.by = "earlyRelapse", idents = curr_idents,
            y.max = y_max, # add the y-axis maximum value - otherwise p-value hidden
            assay = "RNA", slot = "data"
    ) + stat_compare_means(comparisons = test_sign, label = "p.format")
  }
